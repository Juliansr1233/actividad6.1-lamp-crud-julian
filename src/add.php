...
//Comprueba si existen campos vacíos
	...
	{
		if(empty($posicion)) {
			echo "<div>Campo posicion vacío.</div>";
		}
		...
		if(empty($PJ)) {
			echo "<div>Campo PartidosJugados vacío.</div>";
		}

		if(empty($GF)) {
			echo "<div>Campo GolesFavor vacío.</div>";
		}
		
		if(empty($GC)) {
			echo "<div>Campo GolesContra vacío.</div>";
		}
		
		if(empty($DG)) {
			echo "<div>Campo GolesDirecto vacío.</div>";
		}
...
//Prepara una sentencia SQL para su ejecución. En este caso el alta de un registro de la BD.		
		$stmt = mysqli_prepare($mysqli, "INSERT INTO LaLigaJSR (posicion,name,PJ,GF,GC,DG) VALUES(?,?,?,?,?,?)");
...
*/		
		mysqli_stmt_bind_param($stmt, "ssiiii", $posicion, $name, $PJ, $GF, $GC, $DG);
